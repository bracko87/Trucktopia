...
export default function Registration() {
  ...
  const { register, gameState } = useGame()
      ...
      <Card className="w-full max-w-md bg-slate-800/90 backdrop-blur-sm border-slate-700 shadow-2xl relative z-10">
        <CardHeader className="space-y-1 text-center pb-8">
          ...
          <CardDescription className="text-slate-400">
            Join the elite trucking community
          </CardDescription>
        </CardHeader>
                ...
                <div className="space-y-2 mt-3 p-3 bg-slate-700/50 rounded-lg border border-slate-600">
                  ...
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <CheckCircle 
                        className={`h-3 w-3 ${req.met ? 'text-green-500' : 'text-slate-500'}`} 
                      />
            ...
            <div className="space-y-2">
              ...
              {formData.confirmPassword && (
                <div className="flex items-center gap-2 mt-2">
                  <CheckCircle 
                    className={`h-3 w-3 ${formData.password === formData.confirmPassword ? 'text-green-500' : 'text-red-500'}`} 
                  />
